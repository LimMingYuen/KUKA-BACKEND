-- =====================================================
-- Migration: AddTemplateCategories
...
-- 2. Create unique index on TemplateCategories.Name
CREATE UNIQUE INDEX [IX_TemplateCategories_Name] ON [TemplateCategories] ([Name]);

-- 3. Add CategoryId column to SavedCustomMissions
...
ALTER TABLE [SavedCustomMissions]
    ADD CONSTRAINT [FK_SavedCustomMissions_TemplateCategories_CategoryId]
    FOREIGN KEY ([CategoryId])
    REFERENCES [TemplateCategories] ([Id])
    ON DELETE SET NULL;

-- 6. Record migration in EF Core history table
